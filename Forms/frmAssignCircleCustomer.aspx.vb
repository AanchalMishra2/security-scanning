...
Partial Class Forms_frmAssignCircleCustomer
    ...
    Private Sub FillRolName()
        Try
            ...
            arrParameter(0, 1) = ""
            DrRolName = objClsdb.ExecProc_getDataReder("D_SP_GET_CUSTOMER_FOR_DROPDOWN", arrParameter)
            ddlcustomer.Items.Clear()
            If DrRolName.HasRows Then
                ddlcustomer.DataSource = DrRolName
    ...
    Private Sub FillRightList(ByVal UserId As Integer)
        Try
            ...
            cmdRolModule.CommandText = "D_GET_AVAILABLE_CIRCLES_FOR_CUSTOMER"
    ...
    Private Sub FillAssignedRights(ByVal UserId As Integer)
        Try
            ...
            Dim cmdRolModule As SqlCommand
            cmdRolModule = New SqlCommand
            cmdRolModule.CommandText = "D_GET_ASSIGNED_CIRCLES_TO_CUSTOMER"
            cmdRolModule.CommandType = CommandType.StoredProcedure
            cmdRolModule.Connection = objClsdb.GetConnection()
            'Role ID
            cmdRolModule.Parameters.Add("@intCustomerId", SqlDbType.Int)
    ...
    Protected Sub ddlcustomer_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlcustomer.SelectedIndexChanged
        Try
            FillRightList(ddlcustomer.SelectedItem.Value)
            FillAssignedRights(ddlcustomer.SelectedItem.Value)
        Catch ex As Exception
            Functions.catch_Exception(ex, Page.ToString.Trim)
        End Try
    End Sub
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            cmdRolModule = New SqlCommand
            cmdRolModule.CommandText = "D_SP_DELETE_ASSIGNED_CIRCLES_FROM_CUSTOMER"
            cmdRolModule.CommandType = CommandType.StoredProcedure
            cmdRolModule.Connection = ObjCon.GetConnection()

            ''' Role Name
            cmdRolModule.Parameters.Add("@intCustomerId", SqlDbType.BigInt, 20)
            cmdRolModule.Parameters(0).Value = ddlcustomer.SelectedValue
            cmdRolModule.ExecuteNonQuery()
            ObjCon.CloseConnection()
            For intI = 0 To lstAssign.Items.Count - 1
                cmdRolModule = New SqlCommand
                cmdRolModule.CommandText = "D_SP_INSERT_ASSIGNED_CIRCLE_TO_CUSTOMER_DTL"
                cmdRolModule.CommandType = CommandType.StoredProcedure
                cmdRolModule.Connection = ObjCon.GetConnection()
