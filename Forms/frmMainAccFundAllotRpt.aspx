<%@ Page Language="VB" MasterPageFile="~/TestMaster.master" AutoEventWireup="false" CodeFile="frmMainAccFundAllotRpt.aspx.vb" Inherits="Forms_frmMainAccFundAllotRpt" title="Energy Tracker" %>
...
<div class="strip">
        <h1>
           Main Account Transaction Report
           </h1>
    </div>
    <div class="clear">
    </div>
        ...
        <table>
            ...
            <tr>
             <td>
                ...
                <td>
                    <asp:DropDownList CssClass="select_option" ID="ddlvendor" runat="server" AutoPostBack="true"
                        Width="165px">
                    </asp:DropDownList>
                ...
                <td>
                    <asp:Label ID="Label1" Text="Main Account : " runat="server"></asp:Label>
                </td>
                <td>
                    <asp:DropDownList CssClass="select_option" ID="ddlmainacc" runat="server" AutoPostBack="false"
                        Width="165px">
                    </asp:DropDownList>
                    <asp:RequiredFieldValidator ID="reqmainacc" ControlToValidate="ddlmainacc" InitialValue="0"
                        ErrorMessage="Select Vendor" ValidationGroup="rpt" runat="server">*</asp:RequiredFieldValidator>
                    <cc1:ValidatorCalloutExtender ID="vcemainacc" TargetControlID="reqmainacc" runat="server">
                    </cc1:ValidatorCalloutExtender>
            ...
            <asp:Button CssClass="input_btn input_text_x" OnClientClick=" Javascript:openWindow()" ValidationGroup="rpt"  ID="btndetailed" runat="server" Text="View"  />
...
//                if (param=="D"){window.open("frmVendorRPT.aspx?param=" + document.getElementById('<%=ddlCustomer.ClientID%>').value + "&c=" +document.getElementById('<%=ddlCircle.ClientID%>').value + "&v=" +document.getElementById('<%=ddlvendor.ClientID%>').value + "&f=" + document.getElementById('<%=txt_frmdt.ClientID%>').value+"&t=" + document.getElementById('<%=txt_todt.ClientID%>').value +"&Mode=D");}
                //if(param=="S"){
                var mainaccno=  document.getElementById('<%=ddlmainacc.ClientID%>').options[document.getElementById('<%=ddlmainacc.ClientID%>').selectedIndex].text;
                window.open("frmMainAccFundAllotRPTNew.aspx?param=" + document.getElementById('<%=ddlCustomer.ClientID%>').value + "&c=" +document.getElementById('<%=ddlCircle.ClientID%>').value + "&v=" +document.getElementById('<%=ddlvendor.ClientID%>').value + "&f=" + document.getElementById('<%=txt_frmdt.ClientID%>').value+"&t=" + document.getElementById('<%=txt_todt.ClientID%>').value +"&m="+ mainaccno +"");
                //}
