<%@ Page Language="VB" MasterPageFile="~/TestMaster.master" AutoEventWireup="false" CodeFile="frmPurchaseSummeryRpt.aspx.vb" Inherits="Forms_frmPurchaseSummeryRpt" title="Energy Tracker" %>
...
<div class="strip">
        <h1>
           Circle Wise Purchase Summary Report</h1>
    </div>
    <div class="clear">
    </div>
    ...
    function openWindow()
       ...
       // debugger;
        ...
        if (Page_IsValid) {
            // do something
             window.open("frmPurchaseSummaryRPT.aspx?param=" + document.getElementById('<%=ddlCustomer.ClientID%>').value + "&c=" +document.getElementById('<%=ddlCircle.ClientID%>').value +"&f=" + document.getElementById('<%=txt_frmdt.ClientID%>').value+"&t=" + document.getElementById('<%=txt_todt.ClientID%>').value +"");
            //alert('Page is valid!');                
                }
        else {
            // do something else
