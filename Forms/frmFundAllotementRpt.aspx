<%@ Page Language="VB" MasterPageFile="~/TestMaster.master" AutoEventWireup="false" CodeFile="frmFundAllotementRpt.aspx.vb" Inherits="Forms_frmFundAllotementRpt" title="Energy Tracker" %>
<%@ Register Assembly="AjaxControlToolkit" Namespace="AjaxControlToolkit" TagPrefix="cc1" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
<div class="strip">
        <h1>Fund Allotment Report</h1>
    </div>
    <div class="clear">
    </div>
    ...
    <asp:Panel ID="pnl_controls" runat="server">
        <table style="width: 543px">
            ...
            <asp:Button CssClass="input_btn input_text_x"  OnClientClick=" Javascript:openWindow('S')" ValidationGroup="rpt"  ID="Button1" runat="server" Text="Summary"   />
            <asp:Button CssClass="input_btn input_text_x" OnClientClick=" Javascript:openWindow('P')" ValidationGroup="rpt"  ID="Button2" 
                    runat="server" Text="Percentage"   />
             <asp:Button CssClass="input_btn input_text_x"   ID="btnreset" runat="server" 
                    Text="Reset" Width="135px"  />
    ...
    function openWindow(param)
       ...
       // debugger;
        ...
        if (Page_IsValid) {
                if (param=="D"){window.open("frmFundAllotementAllRPT.aspx?param=" + document.getElementById('<%=ddlCustomer.ClientID%>').value + "&c=" +document.getElementById('<%=ddlCircle.ClientID%>').value +"&f=" + document.getElementById('<%=txt_frmdt.ClientID%>').value+"&t=" + document.getElementById('<%=txt_todt.ClientID%>').value +"&Mode=D");}
                if(param=="S"){window.open("frmFundAllotementAllRPT.aspx?param=" + document.getElementById('<%=ddlCustomer.ClientID%>').value + "&c=" +document.getElementById('<%=ddlCircle.ClientID%>').value +"&f=" + document.getElementById('<%=txt_frmdt.ClientID%>').value+"&t=" + document.getElementById('<%=txt_todt.ClientID%>').value +"&Mode=S");}
                if(param=="P"){window.open("frmFundAllotementAllRPT.aspx?param=" + document.getElementById('<%=ddlCustomer.ClientID%>').value + "&c=" +document.getElementById('<%=ddlCircle.ClientID%>').value +"&f=" + document.getElementById('<%=txt_frmdt.ClientID%>').value+"&t=" + document.getElementById('<%=txt_todt.ClientID%>').value +"&Mode=P");}
